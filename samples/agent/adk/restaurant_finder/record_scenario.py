...
async def record_query(agent, query, session_id):
    """
...
async def main():
    ...
    search_messages = await record_query(agent, search_query, session_id)
    if search_messages:
        ...
        if first_restaurant:
            ...
            booking_messages = await record_query(agent, booking_query, session_id)
            if booking_messages:
            ...
            confirmation_messages = await record_query(agent, confirmation_query, session_id)
